 ...
 *
 * @param  {Function|Function[]} input  Type contructor or array of type constructors used in Vue's prop defintions
 * @return {string}                     Plain text string version, arrays become human lists
 */
export const typeToString = function (input) {
  const typeMap = {
    [Array]: 'Array',
    [Boolean]: 'Boolean',
    [Number]: 'Number',
  ...
  input = arrayTypeToString(input);
  input = typeMap[input] || input;
  return input;
};
