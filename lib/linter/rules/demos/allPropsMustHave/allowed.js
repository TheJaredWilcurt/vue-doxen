...
export const allPropsMustHaveAllowed = {
    ...
    if (linterSettings.demos.allPropsMustHave[key]) {
      for (const demoName in demos) {
        ...
        for (const propName of allPropNames) {
          const valid = (
            validateArrayOfPrimitives(demo?.props?.[propName], key) ||
            validateArrayOfPrimitives(demo?.propsToDemo?.[propName], key) ||
            validateArrayOfPrimitives(demo?.component?.props?.[propName], key) ||
            validateArrayOfPrimitives(demo?.component?.propsToDemo?.[propName], key)
          );
          if (!valid) {
  ...
  title: 'demos.allPropsMustHave.allowed',
