...
/**
 * A container for console colors that can be
 * overwritten to avoid CI issues during tests.
 */
let $c = {
  bold,
  cyanBright,
...
if (globalThis.process?.env?.VITEST) {
  const x = (y) => y;
  /**
   * Override the color functions when tests
   * run so their are now embedded color codes
   * triggering a mismatch in unit tests.
   */
  $c = {
    bold: x,
    cyanBright: x,
