...
export const serializeDemos = async function (demos, options) {
  ...
  if (playwrightOptions) {
    ...
    if (unresolvedDemos.length) {
      console.log(
        'serializeDemos: Extracting component-based fields via Playwright for ' +
        unresolvedDemos.length + ' demo(s). This may take a while...'
      );
      ...
      try {
        for (let i = 0; i < unresolvedDemos.length; i++) {
          const demoName = unresolvedDemos[i];
          ...
          // Wait for at least one data-doxen-serialize element to render.
          // Uses a short timeout — not every page will have one.
          await page.waitForSelector('[data-doxen-serialize]', { timeout: 5000 }).catch(() => {});
          ...
          for (const fieldName of TEXT_FIELDS) {
            ...
            if (el) {
              ...
              if (text && text.trim()) {
                ...
                extracted.push(outKey);
              }
            }
          }
          const status = extracted.length ?
            'extracted: ' + extracted.join(', ') :
            'no fields found';
          console.log(
            '  [' + (i + 1) + '/' + unresolvedDemos.length + '] ' +
            demoName + ' — ' + status
          );
        }
      } finally {
