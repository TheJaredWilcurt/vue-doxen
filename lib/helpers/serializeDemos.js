...
/**
 * Dynamically imports a module, preventing Vite from including it in a bundle.
 *
 * @param  {string}          moduleName  Name of the module to import
 * @return {Promise<object>}             The resolved module import
 */
const importWithoutVite = (moduleName) => {
...
export const serializeDemos = async function (options) {
  ...
  statusLog('Vue-Doxen: Normalizing and flattening demos.');
  ...
  for (const demoName in flattenedDemos) {
    ...
    for (const topLevelValue of topLevelValues) {
      if (
        typeof(demo.mcp) === 'object' &&
        Object.hasOwn(demo.mcp, topLevelValue)
      ) {
        json[demoName][topLevelValue] = demo.mcp[topLevelValue];
      } else if (demo[topLevelValue]) {
    ...
    /* MCP Overrides */
    if (demo.mcp) {
      json[demoName] = _merge(json[demoName], demo.mcp);
    }
  }
  ...
  if (Object.keys(needsDomCheck).length) {
    ...
    if (!url) {
      ...
      try {
        ...
        const playwright = await importWithoutVite('playwright');
          ...
          if (element) {
            ...
            const domContents = await element.evaluate((node) => {
              ...
              return {
                ...
                innerText: (cloneText.innerText)
                  .replaceAll('REPLACE_WITH_A_RETURN', '\n')
