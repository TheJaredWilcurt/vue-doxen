 ...
 * (see example).
 * BUG: If the slot has default contents, the below code does not handle that and will inadvertently create a slot prop called "}"
 * TODO: Fix bug
 *
 * @example
 * {
...
export const getSlotDataFromComponent = function (component) {
  ...
  function findSlotProps (slotDetails) {
    ...
    return slotDetails
      ...
      ?.map((value) => {
        // TODO: Temp fix until https://github.com/TheJaredWilcurt/vue-doxen/issues/34 is addressed
        if (value === '}') {
          return '';
        }
        return value;
  ...
  function setSlotData (slotName, slotProps) {
    if (slotName) {
      slotData[slotName] = {
  ...
  function loopEachSlotLine (lines, quoteMark) {
    ...
    lines?.forEach((line) => {
      ...
      const slotName = slotDetails?.split(quoteMark)?.[1];
      const slotProps = findSlotProps(slotDetails);
      setSlotData(slotName, slotProps);
    });
  }

  try {
    // The template render function as a sting
    const render = component?.render?.toString();
    // Is the component built/minified/uglified (result of `vite build`)
    const isBuiltComponentWithSlots = render?.includes('.$slots,`');
    // Is the component being ran locally (with `vite dev`)
    const isDevComponentWithSlots = render?.includes('.$slots,');
    // Look for all slots ignoring stuff before first slot
    const lines = render?.split('.$slots,')?.toSpliced(0, 1);

    if (isBuiltComponentWithSlots) {
      loopEachSlotLine(lines, '`');
    } else if (isDevComponentWithSlots) {
      loopEachSlotLine(lines, '"');
    }
  } catch {}
  return slotData;
      ...
      if (
        ...
        for (const propKey in props[propName]) {
          ...
          if (
            ...
            // Likely not needed, but keeping it for saftey
...
/**
 * We want to let users define and override the prop types in their
  ...
  for (const propName in props) {
    ...
    if (prop.component) {
      ...
      playgroundProps[propName] = {
        component: components.deprecatedProp,
        props: {
          styleTokens
        }
      };
