...
// IMPORTANT!
// Do not use aliased imports, this file is imported by doxenLinter,
// which does not get built by Vite. It is used directly.

/**
 * Converts any native built in constructor to the string of the constructor name.
 * Example: String or Boolean to 'String' or 'Boolean'.
 *
 * @param  {function} obj  Any object that is a potentially native constructor.
 * @return {string}        The constructor name, or undefined, if invalid.
 */
export const nativeBuiltInToString = function (obj) {
  const nativeConstructors = [];
  const buildNativeConstructorsList = function () {
    /**
     * https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects
     *
     * NOTE: Not all global objects listed there can be combined with an empty string to
  ...
  return getNativeBuiltInName(obj);
};
 ...
 * @param  {number}  [indentation=2]      Number of spaces in a one level indent. Defaults to 2.
...
export const serializeJavaScript = function (jsObject, indentation, fullFunction) {
  ...
  const isDomNode = function (element) {
    try {
      // Using W3 DOM2 (works in modern browsers)
      return element instanceof HTMLElement;
    } catch {
      // Environments without HTMLElement will throw an exception.
      // Use IE7+ fallback
      /* v8 ignore next */
      return (
  ...
  let htmlSerializer;
  try {
    // Not all environments have XMLSerializer
    htmlSerializer = new XMLSerializer();
    /* v8 ignore next */
  } catch {}
  const prettyDomNode = function (value) {
    let serialized = '';
    /* v8 ignore else */
    if (htmlSerializer && isDomNode(value)) {
      try {
        const xmlns = ' xmlns="http://www.w3.org/1999/xhtml"';
        serialized = htmlSerializer.serializeToString(value)?.replace(xmlns, '');
      } catch {}
    }
    /* v8 ignore next */
  ...
  if (isDomNode(jsObject)) {
    return prettyDomNode(jsObject);
  }
   ...
   * @return {string}      The human readble event type or undefined
   */
  ...
  const valueType = function (object) {
    ...
    if (nativeBuiltInToString(object)) {
  ...
  const pretty = function (element, indent, fromArray) {
    ...
    if (visited.indexOf(element) === -1) {
      switch (type) {
        case 'DOM_NODE':
          return fromArray + '\'' + prettyDomNode(element) + '\'';

        case 'NATIVE_BUILT_IN':
          return fromArray + nativeBuiltInToString(element);
