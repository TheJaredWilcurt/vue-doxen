...
export const createOptionsProp = function (COMPONENT_NAME) {
  const allowedOptionValueTypes = ['string', 'number', 'boolean', 'undefined'];
  ...
  if (includeValidator) {
    prop.validator = function (options) {
      ...
      return (
        ...
        existsAndIsObject(options.components.deprecatedProp) &&
        existsAndIsObject(options.components.deprecationBanner) &&
        ...
        existsAndIsObject(options.components.numberField) &&
        ...
        existsAndIsObject(options.components.rangeSlider) &&
