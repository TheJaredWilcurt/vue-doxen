...
import { nativeBuiltInToString } from '../lib/helpers/serializeJavaScript.js';
 ...
 */
export const endsInPeriodOrNotAString = function (value) {
...
/**
 * Validates a key on a root object exists and is a function.
 *
 * @param  {object}  root  Any nested object
 * @param  {string}  key   A key to check on the object
 * @return {boolean}       True = valid, false = invalid
 */
export const validateFunctionOrUndefined = function (root, key) {
  return !!(
    keyExists(root, key) &&
    ['function', 'undefined'].includes(typeof(root[key]))
  );
};
/**
...
/**
 * Checks the shape of a given object for evidence that it is a Vue component.
 *
 * @param  {object}  demoOrComponent  A demo, Vue component, or literally anything else.
 * @return {boolean}                  True = it is probably a Vue component
 */
export const isComponent = function (demoOrComponent) {
  if (
    typeof(demoOrComponent) !== 'object' ||
    Array.isArray(demoOrComponent)
  ) {
    return false;
  }
  if (
    demoOrComponent.render &&
    typeof(demoOrComponent.render) === 'function'
  ) {
    return true;
  }
  if (
    demoOrComponent.setup &&
    typeof(demoOrComponent.setup) === 'function'
  ) {
    return true;
  }
  if (
    demoOrComponent.__file &&
    typeof(demoOrComponent.__file) === 'string' &&
    demoOrComponent.__file.endsWith('.vue')
  ) {
    return true;
  }
  return false;
...
/**
 * Gets an array of all slot names for a given demo/component.
 *
 * @param  {DEMO}     demo  Any demo object or component.
 * @return {string[]}       Array of de-duped slot names.
 */
export const getAllSlotNames = function (demo) {
  let slotName = [];
  /* v8 ignore else */
  if (demo && typeof(demo) === 'object') {
    slotName = getAllPropNamesOrEmitNames([
      demo.slots,
      demo.slotsToDemo,
      demo.component?.slots,
      demo.component?.slotsToDemo
    ]);
  }
  return slotName;
};
 ...
 */
export const allEmitsMustHave = function (demos, linterSettings, errors, key, messageSuffix, ruleName) {
...
/**
 * Detects if a given value is a container of a component.
 *
 * @param  {object}  obj  Any value.
 * @return {boolean}      True means it is an object with a component inside.
 */
export const isComponentContainer = function (obj) {
  return (
    obj &&
    typeof(obj) === 'object' &&
    !Array.isArray(obj) &&
    Object.hasOwn(obj, 'component') &&
    isComponent(obj.component)
  );
};

/**
 * Validates if a value is undefined, a Vue component, or a string with length.
 *
 * @param  {undefined|object|string} value  The value to validate.
 * @return {boolean}                        True = validated to be one of the 3 types.
 */
export const undefinedOrIsComponentOrStringWithLength = function (value) {
  return (
    value === undefined ||
    isComponentContainer(value) ||
    (
      typeof(value) === 'string' &&
      value.length
    )
