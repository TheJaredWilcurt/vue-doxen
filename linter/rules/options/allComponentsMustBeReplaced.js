    ...
    if (linterSettings.options.allComponentsMustBeReplaced) {
      if (
        ...
        for (const componentName in defaultOptions.components) {
          const doxenName = defaultOptions.components[componentName];
          const replacementComponent = options.components[componentName];
          if (doxenName === replacementComponent?.name) {
            const message = 'The ' + componentName + ' component in your Vue-Doxen options object should not be named ' + doxenName + '.';
            errors.push({
              ruleName,
              demoName,
              message
